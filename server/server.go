...
func Run() *http.Server {
	log.Println("INFO: Sever: Welcome to PriceTracker, the server that would save you money!")
	...
	go func() {
		log.Printf("INFO: Sever: Listening at http://localhost:%s\n", port)
		if err := srv.ListenAndServe(); err != nil {
			// cannot panic, because this probably is an intentional close
			log.Printf("ERROR: Sever: ListenAndServe error: %s", err)
		}
	}()
	return srv
}
